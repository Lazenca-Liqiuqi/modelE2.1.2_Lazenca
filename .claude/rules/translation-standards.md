# ModelE2.1.2_Lazenca 翻译规范

## 📋 规范适用范围

本规范适用于ModelE2.1.2_Lazenca项目的所有翻译工作，包括：
- **ModelDescription目录**：技术文档翻译
- **HOWTO目录**：开发者指南文档翻译
- **misc目录**：配置文件和辅助文档翻译
- **Fortran代码**：源代码注释翻译
...
为避免格式规范冲突，本项目采用以下统一格式标准：

### 标准格式（推荐）
- **标题格式**：`# English / 中文` 或分两行 `# English \n # 中文`
- **段落格式**：英文在上，中文在下，中间空一行
- **不使用【中文翻译】标记**：保持简洁，通过上下文区分
- **代码块保留**：代码内容不翻译，注释可翻译

### 特殊情况（保留）
- **Fortran代码注释**：可使用`! English` + `! 中文`格式
- **脚本文件注释**：可使用`# English` + `# 中文`格式
...
## 十四、经验教训总结（基于任务#13/#15/#19审查）

### 14.1 特殊术语保留英文原则

**问题**：Q-flux_mixed_layer_model.md中"spin up run"译为"旋合运行"
- 在气象学和流体力学中，"旋合"通常指"辐合/收敛"（convergence）
- 与同段落中的"ocean heat convergence/海洋热辐合"并列时，造成概念混淆
- 容易误导读者理解为"旋转式的辐合"而非"模型预平衡过程"

**教训**：对于跨领域容易引起歧义的专业术语，应保留英文并提供中文解释
**修正**：
- 保留英文：spin-up运行
- 添加中文释义：（预平衡运行）或（自旋预热运行）
- 避免使用与物理过程不符的汉字直译

**适用场景**：
- 数值模拟初始化过程：spin-up、spin up、warm-up等
- 跨领域术语：同一中文词在不同领域有不同含义时
- 新兴术语：学术界尚未形成标准译名的术语

### 14.2 标题译名与词典一致性

**问题**：Diagnostics.md标题使用"诊断"，与词典v1.4条目"诊断输出"不一致
- 词典条目：Diagnostics → 诊断输出（terminology-dictionary.md:142）
- 文件标题：# Diagnostics / 诊断
- 正文中：诊断/诊断输出混用

**教训**：标题译名必须与术语词典保持一致，词典是唯一的术语标准来源
**修正**：
- 标题改为：# Diagnostics / 诊断输出
- 正文中统一使用"诊断输出"或"诊断"择一
- 检查所有文件标题是否与词典条目一致

### 14.3 海洋与海冰术语特点

**问题**：第四批涉及海洋科学和海冰科学，出现大量新术语
- 海洋术语：ocean heat convergence、mixed layer、spin-up run等
- 海冰术语：frazil ice、lead fraction、melt pond、sigma-coordinate等
- 系统术语：time stepping loop、advective tendencies、budget page diagnostics等

**教训**：不同科学领域的术语需要专业背景知识，不能按通用含义翻译
**关键术语**（15个新术语需补录）：
...
- [ ] 注意译词在专业领域的其他含义
- [ ] 按句拆分时保持顺序和格式一致
- [ ] 检查同根术语的译词一致性（如conductance系列）
- [ ] 注意位置概念的准确性（如cloud top/base、above/below）
- [ ] **新增**：对于易混淆的跨领域术语，优先保留英文并添加中文释义
- [ ] **新增**：检查标题译名是否与词典条目一致
...
| 错误类型 | 示例 | 正确做法 |
|---------|------|----------|
| 同根术语不一致 | "传导"与"导度"混用（conductance） | 同根术语统一译词 |
| 术语辨识度不足 | "下沉循环"与"下沉气流"混淆 | 明确物理过程，增加区分度 |
| 位置概念错误 | "云底"应为"云顶" | 准确理解空间关系 |
| 口语化表达 | "根深"过于简略 | 使用"根系深度" |
| 拼写处理 | 直接更正，无说明 | 更正+标注"（原文拼写：xxx）" |
...
**创建日期**: 2026-02-02
**版本**: v1.4
**最后更新**: 2026-02-05（第1-2批次HOWTO翻译审查后）
**更新内容**:
- 新增第十五章：第1-2批次HOWTO术语（约60个新术语）
- 统一格式标准声明：明确项目整体采用中英对照叠放格式，不使用【中文翻译】标记
- 规范适用范围扩展：覆盖ModelDescription、HOWTO、misc目录及Fortran代码
